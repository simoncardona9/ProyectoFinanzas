...
| `POST /transactions` | body: `date`, `type`, `status`, `amountMinor`, `currency`, `accountId`, `categoryId`, `description`, optional links and flags | Create income, expense, transfer, debt-payment, or adjustment. |
...
| Method and path | Parameters | Purpose |
|---|---|---|
| `GET /dashboard` | `period` (`YYYY-MM`), `baseCurrency`, `exchangeRateId` optional | Return spendable cash, projected cash, income, obligations, tax reserves, and debt summary. |
