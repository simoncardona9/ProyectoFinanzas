...
2. Register a confirmed `USD` → `UYU` rate of `42.75`, dated `2026-09-06`,
   with a recognizable synthetic source.
...
The selected rate’s date, source, and kind must be visible with the UYU
exposure. Without selecting a rate, the report must not combine UYU and USD or
show a total UYU exposure. Confirm that the debt detail shows the same
remaining USD balance and equivalence when the identical rate is selected.
