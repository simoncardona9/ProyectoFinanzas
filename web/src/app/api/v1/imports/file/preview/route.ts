...
export async function POST(request: Request) {
  try {
    ...
    const declaredPeriod =
      String(form.get("declaredPeriod") ?? "").trim() || undefined;
    if (!(file instanceof File) || !file.name)
      return Response.json(
        {
          error: {
            code: "VALIDATION_ERROR",
            message: "Selecciona un archivo CSV, XLSX o XLSM.",
          },
        },
        { status: 400 },
      );
      ...
      return Response.json(
        {
          error: {
            code: "VALIDATION_ERROR",
            message: "El período declarado debe tener formato AAAA-MM.",
          },
        },
        { status: 400 },
      );
      ...
      return Response.json(
        {
          error: {
            code: "VALIDATION_ERROR",
            message: "Solo se admiten archivos .csv, .xlsx y .xlsm.",
          },
        },
        { status: 400 },
      );
