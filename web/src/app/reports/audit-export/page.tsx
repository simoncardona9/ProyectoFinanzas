...
export default async function AuditExportPage({
  ...
  if (!["owner", "accountant"].includes(context.membership.role))
    redirect("/settings");
    ...
    <main className="flex flex-1 justify-center bg-zinc-50 p-6">
      <section className="w-full max-w-6xl rounded-2xl bg-white p-8 shadow-sm ring-1 ring-zinc-950/5">
        ...
        <p className="mt-2 text-zinc-600">
          La exportación incluye registros financieros fechados, no lotes de
          importación ni archivos originales. Cada descarga queda registrada en
          la auditoría.
        </p>
        {!parsed.success && (
          <p className="mt-3 rounded bg-amber-50 p-3 text-sm text-amber-900">
            Los filtros de auditoría no son válidos; se muestra el historial
            reciente.
          </p>
        )}
        ...
        <section className="mt-6 rounded-xl border p-4">
          ...
          <p className="mt-1 text-sm text-zinc-600">
            Incluye movimientos, obligaciones, facturas, deudas y cotizaciones
            por su fecha financiera dentro del rango. No sustituye una copia de
            respaldo de la base de datos.
          </p>
          <form
            action="/api/v1/reports/export"
        ...
        <section className="mt-8">
          ...
          <form className="mt-3 flex flex-wrap items-end gap-3 rounded-xl border p-4">
            ...
            <label className="grid gap-1 text-sm">
              ...
              <input
                name="action"
                defaultValue={supplied.action}
                placeholder="create, export..."
                className="rounded border p-2"
              />
            </label>
            <label className="grid gap-1 text-sm">
              ...
              <input
                name="entityType"
                defaultValue={supplied.entityType}
          ...
          <p className="mt-3 text-sm text-zinc-600">
            {report.meta.total} evento(s) encontrados.
          </p>
              ...
              <tbody>
                {report.events.map((event) => (
                  <tr key={event.id} className="border-t align-top">
                    ...
                    <td className="p-3">
                      {event.actorEmail ?? "Usuario eliminado"}
                    </td>
                    <td className="p-3">{event.action}</td>
                    <td className="p-3">{event.entityType}</td>
                    <td className="max-w-md p-3 break-words">
