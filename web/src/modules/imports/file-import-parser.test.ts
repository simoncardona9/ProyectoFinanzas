...
describe("file import parsers", () => {
  it("maps aliased CSV transaction headers into the canonical bundle", () => {
    ...
    expect(conversion.bundle.source).toMatchObject({
      type: "csv_upload",
      name: "septiembre.csv",
      declaredPeriod: "2026-09",
    });
  ...
  it("includes hidden Excel rows, ignores empty formatted tails, and blocks formula amounts", () => {
    ...
    const conversion = parseExcelImport(
    ...
    expect(conversion.report.sheets[0]).toMatchObject({
      name: "Caja",
      hiddenRows: 1,
      populatedRows: 3,
    });
    expect(conversion.report.issues).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          row: 3,
          severity: "warning",
          message: expect.stringContaining("oculta"),
        }),
        expect.objectContaining({
          row: 4,
          severity: "error",
          message: expect.stringContaining("fórmula"),
        }),
      ]),
    );
  });
  ...
  it("reports unrecognizable populated sheets instead of silently importing them", () => {
    const conversion = parseCsvImport(
      new TextEncoder().encode("Notas,Valor\ntexto,10"),
      "notas.csv",
    );
    expect(conversion.bundle.transactions).toEqual([]);
    expect(conversion.report.issues).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          severity: "warning",
          message: expect.stringContaining("cabecera"),
        }),
        expect.objectContaining({
          severity: "error",
