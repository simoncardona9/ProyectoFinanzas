...
export async function getAuditReport(
  ...
  const result = await auditExportRepository.auditEvents(
...
export async function createFinancialCsvExport(
  context: AuthContext,
  range: FinancialExportQuery,
) {
  requireRole(context, ["owner", "editor", "accountant"]);
  const source = await auditExportRepository.financialRows(
    context.membership.householdId,
    range,
  );
  const rows: FinancialExportRow[] = [
    ...source.transactionRows.map((row) => ({
      record_type: `transaction:${row.type}`,
      id: row.id,
      date: row.date,
      status: row.status,
    ...
    ...source.obligationRows.map((row) => ({
      record_type: "obligation",
      id: row.id,
      date: row.dueDate,
      status: row.status,
      currency: row.currency,
      amount_minor: row.originalAmountMinor,
      category_id: row.categoryId,
      description: row.description,
    })),
    ...source.invoiceRows.map((row) => ({
      ...
      status: row.status,
      currency: row.currency,
      amount_minor: row.grossAmountMinor,
      counterparty: row.clientName,
      description: row.description,
    })),
    ...source.debtRows.map((row) => ({
      record_type: "debt",
      id: row.id,
      ...
      currency: row.currency,
      amount_minor: row.originalAmountMinor,
      counterparty: row.creditorName,
      description: row.description,
    })),
    ...source.rateRows.map((row) => ({
      record_type: "exchange_rate",
      id: row.id,
      date: row.effectiveDate,
      status: row.kind,
      currency: `${row.baseCurrency}/${row.quoteCurrency}`,
      amount_minor: row.rate,
  ...
  await auditExportRepository.recordExport({
